...
colnames(dataset)[5:(4 + n_days)] <- sprintf("d%02d", as.numeric(1:n_days))

dataset <- dataset %>% select(-Long, -Lat) %>% 
...
url2 <- 'https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_daily_reports/'
url2 <- paste0(url2, date, '.csv')
daily <- read_csv(url2)

library(maps)
US <- daily %>% filter(Country_Region == 'US')
mp <- map_data("usa")

p4 <- ggplot() + geom_polygon(data = mp, aes(x=long, y = lat, group = group)) + coord_map() + 
      geom_point(data = US, aes(x = Long_, y = Lat, size = Confirmed), colour = 'red')
